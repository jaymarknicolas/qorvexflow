...
export default function MiniYouTube() {
  ...
  if (currentVideo) {
    return (
      <div
        className={`flex flex-col h-full bg-gradient-to-br ${colors.gradient} backdrop-blur-xl border ${colors.border}   overflow-hidden`}
      >
        ...
        >
          <Radio
            className={`w-3 h-3 ${colors.accent} animate-pulse flex-shrink-0`}
          />
          <p
            className={`flex-1 text-[10px] ${colors.textPrimary} truncate min-w-0`}
          >
            {currentVideo.title}
          </p>
          <button
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-1">
        {QUICK_STREAMS.map((v) => (
          ...
          >
            ...
            <p
              className={`text-[10px] ${colors.textSecondary} truncate flex-1`}
            >
              {v.title}
            </p>
